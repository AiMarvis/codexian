...
describe('SkillStorage', () => {
  it('exports SKILLS_PATH', () => {
    expect(SKILLS_PATH).toBe('.codexian/skills');
  });

  describe('loadAll', () => {
    it('loads skills from subdirectories with SKILL.md', async () => {
      const adapter = createMockAdapter({
        '.codexian/skills/my-skill/SKILL.md': `---
...
Do the thing`,
    ...
    it('loads multiple skills', async () => {
      const adapter = createMockAdapter({
        '.codexian/skills/skill-a/SKILL.md': `---
description: Skill A
---
Prompt A`,
        '.codexian/skills/skill-b/SKILL.md': `---
description: Skill B
...
Prompt B`,
    ...
    it('skips folders without SKILL.md', async () => {
      const adapter = createMockAdapter({
        '.codexian/skills/has-skill/SKILL.md': `---
description: Valid
---
Prompt`,
        '.codexian/skills/no-skill/README.md': 'Just a readme',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
    ...
    it('skips malformed skill and continues loading valid ones', async () => {
      const adapter = createMockAdapter({
        '.codexian/skills/good/SKILL.md': `---
description: Valid
---
Prompt`,
        '.codexian/skills/bad/SKILL.md': 'content',
      });
...
Full prompt`,
    ...
    it('loads skills without frontmatter as content-only', async () => {
      const adapter = createMockAdapter({
        '.codexian/skills/valid/SKILL.md': `---
description: Valid
---
Prompt`,
        '.codexian/skills/invalid/SKILL.md': 'No frontmatter at all',
      });
      const storage = new SkillStorage(adapter);
      const skills = await storage.loadAll();
  ...
  describe('save', () => {
    it('writes skill to correct path', async () => {
      ...
      expect(adapter.ensureFolder).toHaveBeenCalledWith('.codexian/skills/my-skill');
  ...
  describe('delete', () => {
    it('deletes skill file and cleans up directory', async () => {
      const adapter = createMockAdapter({
        '.codexian/skills/target/SKILL.md': `---
description: Target
---
Prompt`,
      });
      ...
      expect(adapter.delete).toHaveBeenCalledWith('.codexian/skills/target/SKILL.md');
      expect(adapter.deleteFolder).toHaveBeenCalledWith('.codexian/skills/target');
    });
  });
});
