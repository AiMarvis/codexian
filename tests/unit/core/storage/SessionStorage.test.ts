...
describe('SessionStorage', () => {
  ...
  describe('getFilePath', () => {
    it('returns correct file path for conversation id', () => {
      ...
      expect(path).toBe('.codexian/sessions/conv-123.jsonl');
    });
  });
  ...
  describe('loadConversation', () => {
    it('returns null if file does not exist', async () => {
      ...
      expect(mockAdapter.exists).toHaveBeenCalledWith('.codexian/sessions/conv-123.jsonl');
    });
  ...
  describe('saveConversation', () => {
    it('serializes conversation to JSONL and writes to file', async () => {
      ...
      expect(mockAdapter.write).toHaveBeenCalledWith(
        '.codexian/sessions/conv-456.jsonl',
        expect.any(String)
      );
  ...
  describe('deleteConversation', () => {
    it('deletes the JSONL file', async () => {
      ...
      expect(mockAdapter.delete).toHaveBeenCalledWith('.codexian/sessions/conv-del.jsonl');
    });
  });
  ...
  describe('listConversations', () => {
    it('returns metadata for all JSONL files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/conv-1.jsonl',
        '.codexian/sessions/conv-2.jsonl',
        '.codexian/sessions/readme.txt', // Should be skipped
      ]);
    ...
    it('skips files that fail to load', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/good.jsonl',
        '.codexian/sessions/bad.jsonl',
      ]);
    ...
    it('truncates long previews', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codexian/sessions/conv-long.jsonl']);
    ...
    it('uses default preview for conversations without user messages', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codexian/sessions/conv-no-user.jsonl']);
    ...
    it('preserves titleGenerationStatus in meta', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codexian/sessions/conv-status.jsonl']);
  ...
  describe('loadAllConversations', () => {
    it('loads full conversation data for all JSONL files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/conv-a.jsonl',
        '.codexian/sessions/conv-b.jsonl',
      ]);
    ...
    it('skips non-JSONL files', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/conv.jsonl',
        '.codexian/sessions/notes.md',
        '.codexian/sessions/.DS_Store',
      ]);
  ...
  describe('hasSessions', () => {
    ...
    it('returns false if no JSONL files exist', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/readme.txt',
        '.codexian/sessions/.gitkeep',
      ]);
  ...
  describe('listNativeMetadata', () => {
    it('returns metadata for .meta.json files without .jsonl counterparts', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        ...
        '.codexian/sessions/legacy.jsonl',
        '.codexian/sessions/legacy.meta.json', // Has JSONL counterpart, skip
      ]);

      // native-1.meta.json and native-2.meta.json have no .jsonl
      // legacy.meta.json has .jsonl counterpart
      mockAdapter.exists.mockImplementation((path: string) => {
        if (path === '.codexian/sessions/native-1.jsonl') return Promise.resolve(false);
        if (path === '.codexian/sessions/native-2.jsonl') return Promise.resolve(false);
        if (path === '.codexian/sessions/legacy.jsonl') return Promise.resolve(true);
  ...
  describe('listAllConversations', () => {
    it('merges legacy and native conversations', async () => {
      ...
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/legacy-1.jsonl',
        '.codexian/sessions/native-1.meta.json',
      ]);
    ...
    it('legacy takes precedence over native with same ID', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/conv-1.jsonl',
        '.codexian/sessions/conv-1.meta.json', // Same ID, should be skipped
      ]);
    ...
    it('native sessions have isNative flag and default preview', async () => {
      mockAdapter.listFiles.mockResolvedValue([
        '.codexian/sessions/native-only.meta.json',
      ]);
  ...
  describe('listConversations - loadMetaOnly edge cases', () => {
    it('handles corrupted message lines in loadMetaOnly', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codexian/sessions/corrupted.jsonl']);
    ...
    it('returns empty when first line (meta) parse fails', async () => {
      mockAdapter.listFiles.mockResolvedValue(['.codexian/sessions/bad-meta.jsonl']);
